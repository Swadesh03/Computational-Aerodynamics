...
% plot(C_x,s_xfn)
% % axis([0 1 0.5 1]);
eps_a = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35];


%% Initializing the state vectors and pressure
for k = 1:length(eps_a)
...
%         Ts(1) = Tt*(1-b1);
        ...
        b1 = ((g-1)*u(1)^2)/((g+1)*a);
        Ts(1) = Tt*(1-b1);
        Ps(1) = Pt*(Ts(1)/Tt)^(g/(g-1));
        pho(1) = Ps(1)/(r*Ts(1));
        E(1) = pho(1)*((Cv*Ts(1))+(0.5*u(1)^2));
        c(1) = sqrt((g*Ps(1))/pho(1));
        Ma(1) = u(1)/c(1);
    end
...
figure(1);
grid on; 
plot(itr_store,log10(error_store),'LineWidth',1);
xlabel('Iterations','FontSize',12);
ylabel('Log(error)','FontSize',12);
title('Convergence plot of Density residual', 'FontSize',14);
legend('Eps = 0.1','Eps = 0.15','Eps = 0.2','Eps = 0.25','Eps = 0.3','Eps = 0.35');
hold on;
% 
% % Density residual
% 
% figure(2);
% plot(itr_store,log10(error_store_pho),'LineWidth',2);
% xlabel('Iterations','FontSize',12);
% ylabel('Log(Density)','FontSize',12);
% title('Density residual vs Number of Interations at M=0.1', 'FontSize',14);
% 
% Pressure distribution across the channel residual
